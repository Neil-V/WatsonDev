    ...
    for (var i in o) {		// iterate throughout the objects at this node
		if (o[i] !== null && typeof(o[i]) == "string") {						// if we find a string node ...
			if (i == "name") {													// and if that object contains a name field ...
				if (isGlobal) applyGlobalHTML(depth, o["name"], o["path"]);		// then we know a name and a path both exists within it; pass those to applyGlobalHTML()
				else applyLocalHTML(depth, o["name"], o["path"]);				// ... pass the same parameters to applyLocalHTML if isGlobal == false
			}
		}
